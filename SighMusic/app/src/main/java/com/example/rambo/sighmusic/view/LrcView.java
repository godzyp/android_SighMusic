...
import com.example.rambo.sighmusic.bean.LrcEntry;
...
public final class LrcView extends View {
    ...
    private void init(AttributeSet attrs) {
        ...
        mTextSize = ta.getDimension(R.styleable.LrcView_lrcTextSize, LrcUtil.sp2px(getContext(), 12));
        mDividerHeight = ta.getDimension(R.styleable.LrcView_lrcDividerHeight, LrcUtil.dp2px(getContext(), 16));
    ...
    private void initEntryList() {
        ...
        try {
            //容易出现ConcurrentModificationException，在遍历时list的执行了remove操作，做造成此问题
            for (int i = 0; i < mLrcEntryList.size(); i++) {
                LrcEntry lrcEntry = mLrcEntryList.get(i);
